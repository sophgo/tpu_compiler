...
//
// MulConst Ops quantization method
//
LogicalResult quantizeInt8MulConstOps(Operation *op) {
  ...
  auto rewriter = Builder(castOp.getContext());
  castOp->setAttr("const_val", rewriter.getF32FloatAttr(float_quant));
  setOpResultType(op->getResult(0),
                  IntegerType::get(op->getContext(), 8, IntegerType::Signed));
...
LogicalResult tpu::MulConstOp::quantizeInt8() {
  LLVM_DEBUG(llvm::errs() << "quantizeInt8: " << getOperationName() << " ["
                          << getOpName() << "]\n";);
  Operation *op = this->getOperation();
  return quantizeInt8MulConstOps(op);
}
...
DECLARE_QUANTIZE_INT8_BYPASS_METHOD(tpu::CopyOp)
...
DECLARE_QUANTIZE_INT8_BYPASS_METHOD(tpu::MatchTemplateOp)
