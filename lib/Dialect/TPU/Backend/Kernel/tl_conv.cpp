...
//
static void conv_la_oc_step(const CviBackendContext &ctx, uint32_t layer_id,
    gaddr_t ga_ifmap, gaddr_t ga_ofmap, gaddr_t ga_filter, gaddr_t ga_perchannel,
    const conv_args_t &args, uint32_t oc_step) {
  assert(oc_step % NPU_NUM == 0);
  ...
  LLVM_DEBUG(
    llvm::errs() << "    conv_la_oc_step: nchw = ("
                 << n << "," << ic << "," << ih << "," << iw << ")"
                 ", ochw = (" << oc << "," << oh << "," << ow << ")"
                 ", g = " << g
                 << "\n                     "
  ...
  tl_ifmap = ctx.lmem_alloc_tensor(ctx.shape_t4(n, ic, ih, iw), CVK_FMT_I8, /*eu_align=*/1);
  tl_ofmap = ctx.lmem_alloc_tensor(ctx.shape_t4(n, oc, oh, ow), CVK_FMT_I8, /*eu_align=*/1);
  tl_filter[0] = ctx.lmem_alloc_tensor(ctx.shape_t4(ic / g, oc_step, kh, kw), CVK_FMT_I8, /*eu_align=*/0);
  tl_filter[1] = ctx.lmem_alloc_tensor(ctx.shape_t4(ic / g, oc_step, kh, kw), CVK_FMT_I8, /*eu_align=*/0);
  int perchannel_size = with_bias ? 9 : 5;
  tl_perchannel = ctx.lmem_alloc_tensor(ctx.shape_t4(1, oc, 1, perchannel_size), CVK_FMT_U8, /*eu_align=*/0);
  ...
  tl_perchannel->shape = ctx.shape_t4(1, oc, 1, perchannel_size);
  tl_perchannel->stride = ctx.tl_default_stride(tl_perchannel->shape, CVK_FMT_I8, /*eu_aign=*/0);
  ctx.tdma_load(tl_perchannel, ga_perchannel);

  tl_ofmap->stride = ctx.tl_default_stride(tl_ofmap->shape, CVK_FMT_I8, 1);
  // split oc
  int flip = 0;
  for (uint32_t n_pos = 0; n_pos < n; n_pos++) {
    cvk_tl_t tl_ifmap_step;
    tl_ifmap_step.start_address = tl_ifmap->start_address + n_pos * tl_ifmap->stride.n;
    tl_ifmap_step.fmt = CVK_FMT_I8;
    tl_ifmap_step.shape = ctx.shape_t4(1, ic, ih, iw);
    tl_ifmap_step.stride = ctx.tl_default_stride(tl_ifmap_step.shape, CVK_FMT_I8, /*eu_align=*/1);
    ...
    for (uint32_t oc_pos = 0; oc_pos < oc; oc_pos += oc_step) {
      ...
      tl_tmp.shape = ctx.shape_t4(1, cur_oc, kh * kw, ic / g);
      tl_tmp.stride = ctx.tl_default_stride(tl_tmp.shape, CVK_FMT_I8, /*eu_align=*/0);
      ctx.tdma_load_stride(&tl_tmp, ga_filter_oc_pos, filter_gstride);
      ...
      //tl_perchannel->shape = ctx.shape_t4(1, cur_oc, 1, 1);
      //tl_perchannel->stride = ctx.tl_default_stride(tl_perchannel->shape, CVK_FMT_I8, /*eu_aign=*/0);
      cvk_tl_t tl_perchannel_oc_pos;
      tl_perchannel_oc_pos.start_address = tl_perchannel->start_address + (oc_pos / NPU_NUM) * perchannel_size;
      tl_perchannel_oc_pos.fmt = CVK_FMT_I8;
      tl_perchannel_oc_pos.shape = ctx.shape_t4(1, cur_oc, 1, 1); // 1? not 9 or 5? very strange
      tl_perchannel_oc_pos.stride = ctx.tl_default_stride(tl_perchannel_oc_pos.shape, CVK_FMT_I8, /*eu_align=*/0);

      // Adjust current shape and stride
      // bmk does not keep eu-align info, user need to update stride if shape changed
      //tl_ofmap->shape = ctx.shape_t4(n, cur_oc, oh, ow);
      //tl_ofmap->stride = ctx.tl_default_stride(tl_ofmap->shape, CVK_FMT_I8, /*eu_aign=*/1);
      cvk_tl_t tl_ofmap_oc_pos;
      uint32_t ofmap_count = align_up(oh * ow, EU_NUM);
      tl_ofmap_oc_pos.start_address = tl_ofmap->start_address + n_pos * tl_ofmap->stride.n +
                                      (oc_pos / NPU_NUM) * ofmap_count;
      tl_ofmap_oc_pos.fmt = CVK_FMT_I8;
      tl_ofmap_oc_pos.shape = ctx.shape_t4(1, cur_oc, oh, ow);
      tl_ofmap_oc_pos.stride = ctx.tl_default_stride(tl_ofmap_oc_pos.shape, CVK_FMT_I8, /*eu_align=*/1);


      if  (!is_dw) {
        // filter shape for tiu != shape for load
        // bmk does not keep eu-align info, user need to update stride if shape changed
        tl_filter[flip]->shape = ctx.shape_t4(ic / g, cur_oc, kh, kw);
        tl_filter[flip]->stride = ctx.tl_default_stride(tl_filter[flip]->shape, CVK_FMT_I8, /*eu_aign*/ 0);
        ...
        uint32_t ifmap_count = align_up(ih * iw, EU_NUM);
        tl_ifmap_oc_pos.start_address = tl_ifmap_step.start_address + (oc_pos / NPU_NUM) * ifmap_count;
        tl_ifmap_oc_pos.fmt = CVK_FMT_I8;
        tl_ifmap_oc_pos.shape = ctx.shape_t4(1, cur_oc, ih, iw);
        tl_ifmap_oc_pos.stride = ctx.tl_default_stride(tl_ifmap_oc_pos.shape, CVK_FMT_I8, /*eu_align=*/1);

        // filter shape for tiu != shape for load
        // bmk does not keep eu-align info, user need to update stride if shape changed
        tl_filter[flip]->shape = ctx.shape_t4(1, cur_oc, kh, kw);
        tl_filter[flip]->stride = ctx.tl_default_stride(tl_filter[flip]->shape, CVK_FMT_I8, /*eu_aign*/ 1);
  ...
  //
  ctx.lmem_free_tensor(tl_perchannel);
  ctx.lmem_free_tensor(tl_filter[1]);
  ctx.lmem_free_tensor(tl_filter[0]);
...
//   - 2x weight buffer for double buffer parallel
//
static void conv_lw_oc_step(const CviBackendContext &ctx, uint32_t layer_id,
  ...
  LLVM_DEBUG(
    llvm::errs() << "    conv_lw_oc_step: nchw = ("
                 << n << "," << ic << "," << ih << "," << iw << ")"
                 ", ochw = (" << oc << "," << oh << "," << ow << ")"
                 ", g = " << g
                 << "\n                     "
  ...
  la_perChannel[1] = align_up(la_perChannel[0] + ls_perChannel, EU_NUM);
  //double ping-pong buffer
  laddr_t la_filter[2];
  uint32_t ls_filter = ceiling_func(oc_step, NPU_NUM) * kh * kw * ic / g;
  la_filter[0] = align_up(la_working + 2 * ls_perChannel, EU_NUM);
  la_filter[1] = align_up(la_filter[0] + ls_filter, EU_NUM);
  ...
  tl_ifmap_step.fmt = CVK_FMT_I8;
  tl_ifmap_step.shape = ctx.shape_t4(n, ic, ih, iw);
  tl_ifmap_step.stride = ctx.tl_default_stride(tl_ifmap_step.shape, CVK_FMT_I8, /*eu_align=*/1);
  for (uint32_t oc_pos = 0; oc_pos < oc; oc_pos += oc_step) {
    uint32_t cur_oc = std::min(oc - oc_pos, oc_step);
    cvk_tl_t tl_filter;
    ...
    tl_filter.shape = ctx.shape_t4(1, cur_oc, kh * kw, ic / g);
    tl_filter.stride = ctx.tl_default_stride(tl_filter.shape, CVK_FMT_I8, /*eu_align=*/0);
    uint64_t ga_filter_oc_pos = ga_filter + oc_pos * kh * kw * ic / g;
    // filter has been transposed from (oc, ic / g, kh, kw) to (1, oc, kh * kw, ic / g)
    cvk_tg_stride_t filter_gstride = {oc * kh * kw * ic / g, kh * kw * ic / g, ic / g};
    if (!compressed_weight) {
      ...
      ts_data.t.base_reg_index = ctx.getTdmaBaseSelectIndexFromGaddr(ga_filter_oc_pos);
    ...
    tl_perchannel.shape = ctx.shape_t4(1, cur_oc, 1, perchannel_size);
    tl_perchannel.stride = ctx.tl_default_stride(tl_perchannel.shape, CVK_FMT_I8, /*eu_aign=*/0);
    ctx.tdma_load(&tl_perchannel, ga_perChannel_oc_pos);
    ...
    tl_ofmap_oc_pos.start_address = tl_ofmap.start_address +
                                    (oc_pos / NPU_NUM) * ofmap_count;
    tl_ofmap_oc_pos.fmt = CVK_FMT_I8;
    tl_ofmap_oc_pos.shape = ctx.shape_t4(n, cur_oc, oh, ow);
    tl_ofmap_oc_pos.stride = ctx.tl_default_stride(tl_ofmap_oc_pos.shape, CVK_FMT_I8, /*eu_align=*/1);
    tl_ofmap_oc_pos.stride.n = tl_ofmap_oc_pos.stride.c * ceiling_func(oc, NPU_NUM);
    if (!is_dw) {
      // tl_filter shape for tiu != shape for load
      tl_filter.shape = ctx.shape_t4(ic / g, cur_oc, kh, kw);
      tl_filter.stride = ctx.tl_default_stride(tl_filter.shape, CVK_FMT_I8, /*eu_aign*/ 0);
      cvk_tiu_convolution_param_t param = {0};
      param.ofmap = &tl_ofmap_oc_pos;
      param.ifmap = &tl_ifmap_step;
      ...
      tl_ifmap_oc_pos.start_address = tl_ifmap_step.start_address +
                                      (oc_pos / NPU_NUM) * ifmap_count;
      tl_ifmap_oc_pos.fmt = CVK_FMT_I8;
      tl_ifmap_oc_pos.shape = ctx.shape_t4(n, cur_oc, ih, iw);
      tl_ifmap_oc_pos.stride = ctx.tl_default_stride(tl_ifmap_oc_pos.shape, CVK_FMT_I8, /*eu_align=*/1);
      tl_ifmap_oc_pos.stride.n = tl_ifmap_oc_pos.stride.c * ceiling_func(oc, NPU_NUM);
      // tl_filter shape for tiu != shape for load
      tl_filter.shape = ctx.shape_t4(1, cur_oc, kh, kw);
      tl_filter.stride = ctx.tl_default_stride(tl_filter.shape, CVK_FMT_I8, /*eu_aign*/ 1);
      cvk_tiu_depthwise_convolution_param_t param = {0};
      param.ofmap = &tl_ofmap_oc_pos;
      param.ifmap = &tl_ifmap_oc_pos;
      param.weight = &tl_filter;
    ...
    if (do_leaky_relu) {
      ...
      assert(oc_step == (uint32_t)NPU_NUM);
      assert(g == 1);
      // assert(n == 1);
      // work space is kh * kw * ic / g
      // need 2 work space, and align down to EU_NUM
      // workload is oh * ow
      uint32_t relu_step = (kh * kw * ic / g / EU_NUM) * EU_NUM;
      ...
      for(int n_step = 0; n_step < (int)n; n_step++) {
        ...
        while (relu_pos < relu_size) {
          ...
          cvk_tl_t tl_relu;
          tl_relu.start_address = tl_ofmap_oc_pos.start_address + relu_pos + n_step * tl_ofmap_oc_pos.stride.n;
          tl_relu.fmt = CVK_FMT_I8;
          tl_relu.shape = ctx.shape_t4(1, cur_oc, 1, relu_cur);
          tl_relu.stride = ctx.tl_default_stride(
              tl_relu.shape, CVK_FMT_I8, /*eu_align=*/1);
          ...
          tl_leaky_relu(ctx, layer_id,
              tl_relu, tl_relu_working,
              rshift_pos, m_i8_pos, rshift_neg, m_i8_neg);
          relu_pos += relu_cur;
        }
      }
    }
    if (do_store) {
      ...
      // we can't store the data for this flip, we store the data of the opposite flip
      if (first) {
        first = false;
      } else {
        cvk_tl_t tl_ofmap_oc_pos_flip_back;
        // uint32_t ofmap_count = align_up(oh * ow, EU_NUM);
        tl_ofmap_oc_pos_flip_back.start_address = tl_ofmap.start_address +  (oc_pos_flip_back / NPU_NUM) * ofmap_count;
        tl_ofmap_oc_pos_flip_back.fmt = CVK_FMT_I8;
        tl_ofmap_oc_pos_flip_back.shape = ctx.shape_t4(n, cur_oc_flip_back, oh, ow);
        tl_ofmap_oc_pos_flip_back.stride = ctx.tl_default_stride(
                                          tl_ofmap_oc_pos_flip_back.shape, CVK_FMT_I8, /*eu_align=*/1);
        tl_ofmap_oc_pos_flip_back.stride.n = tl_ofmap_oc_pos_flip_back.stride.c * ceiling_func(oc, NPU_NUM);
        gaddr_t ga_ofmap_oc_pos_flip_back = ga_ofmap+ oc_pos_flip_back * oh * ow;
        cvk_tg_stride_t ofmap_gstride = {oc * oh * ow, oh * ow, ow};
        ctx.tdma_store_stride(&tl_ofmap_oc_pos_flip_back,
  ...
  if (do_store) {
    ...
    cvk_tg_stride_t ofmap_gstride = {oc * oh * ow, oh * ow, ow};
    ctx.tdma_store_stride(&tl_ofmap_oc_pos_flip_back, ga_ofmap_oc_pos_flip_back,
                          ofmap_gstride);
  }
}
...
void cvi_backend_tl_conv_LA(
  ...
  conv_la_oc_step(ctx, layer_id, ga_ifmap, ga_ofmap,
                  ga_filter, ga_perchannel, args, NPU_NUM);
}
...
void cvi_backend_tl_conv_LW(
  ...
  conv_lw_oc_step(ctx, layer_id, la_ifmap, la_ofmap, la_working,
                  ga_filter, ga_perchannel, args, NPU_NUM,
                  do_store, ga_ofmap,
...
void cvi_backend_tl_conv(
  ...
  LLVM_DEBUG(llvm::errs() << llvm::format(
                  ...
                  "    rshift_len %d\n",
                  layer_id, la_ifmap, la_ofmap, la_weight, la_perchannel, la_working, input_n, input_c, input_h,
                  input_w, output_c, output_h, output_w, group, kh, kw, pad_h_top, pad_h_bottom, pad_w_left,
                  pad_w_right, stride_h, stride_w, result_add, ctrl, rshift, do_bias, do_relu,
                  rshift_len));
  if (do_ic_alignment && (input_c % 2 != 0)) {
    input_c = input_c + 1;
  }
  ...
  cvk_tl_shape_t tl_input_shape = {
      static_cast<uint32_t>(input_n), static_cast<uint32_t>(input_c), static_cast<uint32_t>(input_h),
  ...
  cvk_tl_shape_t tl_output_shape = {
      static_cast<uint32_t>(input_n), static_cast<uint32_t>(output_c), static_cast<uint32_t>(output_h),
  ...
  if (group == input_c && group == output_c && group != 1) {
    tl_weight_shape.n = 1;
    tl_weight_shape.c = static_cast<uint32_t>(output_c);
    tl_weight_shape.h = static_cast<uint32_t>(kh);
    tl_weight_shape.w = static_cast<uint32_t>(kw);
    ...
    tl_weight_shape.n = static_cast<uint32_t>(input_c);
    tl_weight_shape.c = static_cast<uint32_t>(output_c);
    tl_weight_shape.h = static_cast<uint32_t>(kh);
    tl_weight_shape.w = static_cast<uint32_t>(kw);
    tl_weight.start_address = la_weight;
    tl_weight.fmt = CVK_FMT_I8;
    tl_weight.shape = tl_weight_shape;
    tl_weight.stride = ctx.tl_default_stride(tl_weight_shape, CVK_FMT_I8, 0);
  }
  ...
  tl_chl_quan_param.shape = {1, static_cast<uint32_t>(output_c), 1, 1};
  ...
  LLVM_DEBUG(llvm::errs() << "do_leaky_relu/do_conv_relu:"
             << do_leaky_relu << "/" << do_conv_relu << "\n";);
  ...
  if (group == 1) {
    ...
    if (output_c > MAX_TIU_CHL) {
      int i = 2;
      do {
        oc_step = align_up(ceiling_func(output_c, i++), NPU_NUM * EU_NUM);
      } while (oc_step > MAX_TIU_CHL);

      llvm::errs() << "output_c(" << output_c
    ...
    for (int32_t oc_pos = 0; oc_pos < output_c; oc_pos += oc_step) {
      ...
      cvk_tl_shape_t output_shape, weight_shape;
      cvk_tl_t output, weight, perchannel;
      output_shape.n = input_n;
      output_shape.c = static_cast<uint32_t>(cur_oc);
      output_shape.h = static_cast<uint32_t>(output_h);
      output_shape.w = static_cast<uint32_t>(output_w);
      output.start_address = la_ofmap + (oc_pos / NPU_NUM) * tl_output.stride.c;
      output.fmt = CVK_FMT_I8;
      output.shape = output_shape;
      output.stride = tl_output.stride;

      // filter shape for tiu != shape for load
      weight_shape.n = static_cast<uint32_t>(input_c);
      weight_shape.c = static_cast<uint32_t>(cur_oc);
      weight_shape.h = static_cast<uint32_t>(kh);
      weight_shape.w = static_cast<uint32_t>(kw);
      weight.start_address = la_weight + (oc_pos / NPU_NUM) * kh * kw * input_c;
    ...
    int bottom_csize_local = ALIGN(input_h * input_w, EU_NUM);
    ...
    for (int ig = 0; ig < group; ig++) {
      ...
      for (int nidx = 0; nidx < input_n; nidx++) {
        ...
        uint32_t weight_addr = top_start_npu_idx * LOCAL_MEM_SIZE +
                          ((ig * oc) / NPU_NUM) * ic * kh * kw + tl_weight.start_address;
        ...
        uint32_t bias_local_shift =  (ig * oc / NPU_NUM) * ALIGN(bias_size, EU_NUM);
        uint32_t perchannel_addr = top_start_npu_idx * LOCAL_MEM_SIZE
                                   + la_perchannel +  bias_local_shift;

        // input
        cvk_tl_shape_t input_shape = {
            1, static_cast<uint32_t>(ic), static_cast<uint32_t>(input_h), static_cast<uint32_t>(input_w)};
        cvk_tl_t input;
        input.start_address = bottom_addr;
        input.fmt = CVK_FMT_I8;
        input.shape = input_shape;
        input.stride = ctx.tl_default_stride(input_shape, CVK_FMT_I8, 1);  // EU-aligned

        // weight
        cvk_tl_shape_t weight_shape = {static_cast<uint32_t>(ic), static_cast<uint32_t>(oc),
                                                      static_cast<uint32_t>(kh), static_cast<uint32_t>(kw)};
        cvk_tl_t weight;
        weight.start_address = weight_addr;
        weight.fmt = CVK_FMT_I8;
        weight.shape = weight_shape;
        weight.stride = ctx.tl_default_stride(weight_shape, CVK_FMT_I8, 0);  // Not EU-aligned

        // output
        cvk_tl_shape_t output_shape = {
            1, static_cast<uint32_t>(oc), static_cast<uint32_t>(output_h), static_cast<uint32_t>(output_w)};
        cvk_tl_t output;
        output.start_address = top_addr;
        output.fmt = CVK_FMT_I8;
        output.shape = output_shape;
        output.stride = ctx.tl_default_stride(output_shape, CVK_FMT_I8, 1);  // EU-aligned

        // Per-channel quantization
        tl_chl_quan_param.start_address = perchannel_addr;
        tl_chl_quan_param.shape = {1, static_cast<uint32_t>(oc), 1, 1};
        tl_chl_quan_param.stride =
            ctx.tl_default_stride(tl_chl_quan_param.shape, CVK_FMT_I8, 0);
...
void cvi_backend_bf16_tl_conv(
  ...
  LLVM_DEBUG(llvm::errs() << llvm::format(
                  ...
                  "    do_bias %d, do_relu %d\n"
                  "    \n",
                  layer_id, la_ifmap, la_ofmap, la_weight, la_bias, la_working, input_n, input_c, input_h,
                  input_w, output_c, output_h, output_w, group, kh, kw, pad_h_top, pad_h_bottom, pad_w_left,
  ...
  if (group == input_c && group == output_c && group != 1) {
    ...
    tl_weight.fmt = CVK_FMT_BF16;
    tl_weight.shape = tl_weight_shape;
    tl_weight.stride = ctx.tl_default_stride(tl_weight_shape, CVK_FMT_BF16, 0);
  }
  ...
  tl_bias.shape = {2, static_cast<uint32_t>(output_c), 1, 1};
  tl_bias.stride =
