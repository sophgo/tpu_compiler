...
void cvi_backend_tl_mac_const(const CviBackendContext &ctx, uint32_t layer_id,
                              ...
                              gaddr_t working_addr, int n, int c, int h,
                              int w, int multiplier, int const_val,
                              bool do_relu) {
  // input
  cvk_tl_shape_t tl_shape = {static_cast<uint32_t>(n), static_cast<uint32_t>(c),
...
void cvi_backend_bf16_tl_mac_const(const CviBackendContext &ctx,
                                   uint32_t layer_id, laddr_t input_laddr,
                                   laddr_t output_laddr, int n, int c, int h,
                                   int w, float multiplier, float const_val,
                                   bool do_relu) {
  assert(false && "mac const not support bf16 now");
}
