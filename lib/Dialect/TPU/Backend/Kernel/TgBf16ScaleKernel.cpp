...
#define RELU (0)
  ...
  cvk_tg_stride_t ts_bottom_stride =
      ctx.tg_default_stride(ts_bottom_shape, CVK_FMT_BF16);

  // put scale/bias to lmem
  uint32_t lmem_usage = 0;
  cvk_tl_shape_t tl_scale_shape = ctx.tl_shape_t4(1, input_c, 1, 1);
  cvk_tl_t *tl_scale = ctx.lmem_alloc_tensor(tl_scale_shape, CVK_FMT_BF16, 1);
  ctx.tdma_load(tl_scale, scale_gaddr);
  lmem_usage += ctx.lmem_tensor_to_size(tl_scale_shape, CVK_FMT_BF16, 1);

  cvk_tl_t *tl_bias;
  if (do_bias) {
    cvk_tl_shape_t tl_bias_shape = ctx.tl_shape_t4(2, input_c, 1, 1);
    tl_bias = ctx.lmem_alloc_tensor(tl_bias_shape, CVK_FMT_BF16, 1);
    ctx.tdma_load(tl_bias, bias_gaddr);
    lmem_usage += ctx.lmem_tensor_to_size(tl_bias_shape, CVK_FMT_BF16, 1);
  }

  std::vector<CviBackendContext::tiling_info_t> tiles;
  ctx.tiling_packing(tiles, ts_bottom_shape, CVK_FMT_BF16, lmem_usage,
                     CviBackendContext::TilingDimNHW); // keep c
  cvk_tl_shape_t max_shape =
      ctx.tl_shape_t4(tiles[0].n, tiles[0].c, tiles[0].h, tiles[0].w);
  cvk_tl_t *tl_input = ctx.lmem_alloc_tensor(max_shape, CVK_FMT_BF16, 1);
  if (tl_input == nullptr) {
  ...
  for (auto &tile : tiles) {
    ...
    tl_bslice.stride = ctx.tl_default_stride(tl_bslice.shape, CVK_FMT_BF16, 1);
