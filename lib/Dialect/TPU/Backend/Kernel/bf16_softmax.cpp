...
void bf16_softmax_kernel_4d(const CviBackendContext &ctx, uint32_t layer_id,
    ...
    //Allocate input
    cvk_tl_shape_t input_shape = {
        static_cast<uint32_t>(c), static_cast<uint32_t>(h * w),
        static_cast<uint32_t>(1), static_cast<uint32_t>(1)};
    cvk_tl_t *tl_input =
        ctx.lmem_alloc_tensor(input_shape, CVK_FMT_BF16, eu_align);
    ASSERT(tl_input);
    ...
    cvk_tl_shape_t input_transposed_shape = {
        static_cast<uint32_t>(1), static_cast<uint32_t>(h * w),
    ...
    for(int i = 0; i < n; i++) {
        //load input
        gaddr_t inputAddr = ga_input + i * h * w * c * sizeof(uint16_t);
        ctx.tdma_load_bf16(tl_input, inputAddr);

        bool doConcateC = h * w > NPU_NUM;
        if(doConcateC) {
            cvk_tl_t tl_dst;
        ...
        //tl_lut_result = exp(tl_input)
        {
            ...
            tl_exponential_table_answer->start_address, tl_exponential_table_answer_slope->start_address,
            table_thresh_min, table_thresh_max, 1, h * w, 1, c);
        }
        ...
        //Lut reciprocal value
        {
            ...
            ctx, layer_id,
            tl_maxValue->start_address, tl_lut_reciprocal_result->start_address, tl_lut_working->start_address,
            tl_reciprocal_table_answer->start_address, tl_reciprocal_mantissa_table_answer->start_address, 1, h * w, 1, 1);
        }
