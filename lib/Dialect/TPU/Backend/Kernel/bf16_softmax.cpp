...
void bf16_softmax_kernel_2d(const CviBackendContext &ctx, uint32_t layer_id,
    ...
    cvk_tl_shape_t input_shape = {
        static_cast<uint32_t>(outer_size), static_cast<uint32_t>(1),
        static_cast<uint32_t>(1), static_cast<uint32_t>(inner_size)};
    cvk_tl_t *tl_input =
        ctx.lmem_alloc_tensor(input_shape, CVK_FMT_BF16, eu_align);
    ASSERT(tl_input);
    gaddr_t refactor_ga_input = ga_input;
    ctx.tdma_load_bf16(tl_input, refactor_ga_input);
    ...
    tl_enlargeInput.shape = {
        static_cast<uint32_t>(outer_size), static_cast<uint32_t>(1),
        static_cast<uint32_t>(1), static_cast<uint32_t>(parallelC * bf16_euWorkingOneLane)};
    ...
    cvk_tl_shape_t maxValue_shape = {
        static_cast<uint32_t>(outer_size), static_cast<uint32_t>(1),
        static_cast<uint32_t>(NPU_NUM), static_cast<uint32_t>(1)};
    ...
    tl_maxValue.shape = {(uint32_t)outer_size, 1, 1, 1};
    ...
    //                 h_str = 0
    {
      ...
      tl_dst.shape = {
        static_cast<uint32_t>(outer_size), static_cast<uint32_t>(NPU_NUM),
    ...
    //tl_lut_result = exp(tl_parallel_input)
    {
        ...
        table_thresh_min, table_thresh_max, outer_size, parallelC, 1, bf16_euWorkingOneLane);
    }
    ...
    //Lut reciprocal value
    {
        ...
        tl_reciprocal_table_answer->start_address, tl_reciprocal_mantissa_table_answer->start_address, outer_size, 1, 1, 1);
    }
    ...
    //Store to dram
    {
        ...
        ts_data.start_address = ga_output;
...
unsigned int doSplitHeightBf16softmax4D(const CviBackendContext &ctx, int64_t* shape) {
    ...
    while(true) {
        ...
        cvk_tl_shape_t input_shape = {
            static_cast<uint32_t>(c), static_cast<uint32_t>(tileH * w),
            static_cast<uint32_t>(1), static_cast<uint32_t>(1)};
        int inputSize = ctx.lmem_tensor_to_size(input_shape, CVK_FMT_BF16, eu_align);
        cvk_tl_shape_t input_transposed_shape = {
            static_cast<uint32_t>(1), static_cast<uint32_t>(tileH * w),
            static_cast<uint32_t>(1), static_cast<uint32_t>(c)};
        int inputTransposedSize = ctx.lmem_tensor_to_size(input_transposed_shape, CVK_FMT_BF16, eu_align) * 4;
        //transposedInput + lutWorking + lutResult * 2
...
void bf16_softmax_kernel_4d(const CviBackendContext &ctx, uint32_t layer_id,
    ...
    for(int hStepCounter = 0; hStepCounter < hStep; hStepCounter++) {
        ...
        //Allocate input
        cvk_tl_shape_t input_shape = {
            static_cast<uint32_t>(c), static_cast<uint32_t>(tileHeightSize * w),
            static_cast<uint32_t>(1), static_cast<uint32_t>(1)};
        cvk_tl_t *tl_input =
            ctx.lmem_alloc_tensor(input_shape, CVK_FMT_BF16, eu_align);
        ASSERT(tl_input);
        ...
        cvk_tl_shape_t input_transposed_shape = {
            static_cast<uint32_t>(1), static_cast<uint32_t>(tileHeightSize * w),
            static_cast<uint32_t>(1), static_cast<uint32_t>(c)};
