...
unsigned int doSplitHeightBf16softmax2D(const CviBackendContext &ctx, int outerSize, int innerSize) {
    ...
    while(true) {
        ...
        cvk_tl_shape_t input_shape = {
            static_cast<uint32_t>(tiledOuterSize), static_cast<uint32_t>(1),
            static_cast<uint32_t>(1), static_cast<uint32_t>(innerSize)};
        int inputSize = ctx.lmem_tensor_to_size(input_shape, CVK_FMT_BF16, eu_align);

        cvk_tl_shape_t enlargeInputShape = {
            static_cast<uint32_t>(tiledOuterSize), static_cast<uint32_t>(1),
        ...
        cvk_tl_shape_t parallel_input_shape = {
            static_cast<uint32_t>(tiledOuterSize), static_cast<uint32_t>(parallelC),
...
void bf16_softmax_kernel_2d(const CviBackendContext &ctx, uint32_t layer_id,
    ...
    for(int outerSizeCounter = 0; outerSizeCounter < outerSizeStep; outerSizeCounter++) {
        ...
        cvk_tl_shape_t input_shape = {
            static_cast<uint32_t>(workingOutputSize), static_cast<uint32_t>(1),
