...
static void cvi_backend_tg_bf16_reduce_max_chl_kernel(
...
static void cvi_backend_tg_bf16_reduce_min_chl_kernel(
    const CviBackendContext& ctx,
    uint32_t layer_id, gaddr_t ga_input, gaddr_t ga_output,
    int n, int c, int h, int w) {
  cvk_tl_shape_t tl_input_shape = ctx.tl_shape_t4(n, 1, c, h * w);
...
static void cvi_backend_tg_bf16_reduce_min_hw_kernel(
    const CviBackendContext& ctx,
    uint32_t layer_id, gaddr_t ga_input, gaddr_t ga_output,
    int n, int c, int h, int w,
    int kh, int kw) {
  ...
  for (int pos_c = 0; pos_c < c; pos_c += step_c) {
    ...
    // Min
    {
      int mul_const = ctx.convert_fp32_to_bf16(-1.0);
      int relu_enable = 0;
      int shift_bits = 0;
...
static void cvi_backend_tg_bf16_reduce_max_hw_kernel(
    const CviBackendContext& ctx,
    uint32_t layer_id, gaddr_t ga_input, gaddr_t ga_output,
    int n, int c, int h, int w,
    int kh, int kw) {
...
static void cvi_backend_tg_bf16_reduce_mean_hw_kernel(
    const CviBackendContext& ctx,
    uint32_t layer_id, gaddr_t ga_input, gaddr_t ga_output,
    int n, int c, int h, int w,
    int kh, int kw) {
...
void cvi_backend_tg_bf16_reduce_max_kernel(
  ...
  if (axes[0] == 1) {
    // along C axis
    cvi_backend_tg_bf16_reduce_max_chl_kernel(ctx,
                                              layer_id,
                                              ga_input,
                                              ga_output,
                                              n,
    ...
    int kh = (axes[0] == 2) ? h : 1;
    int kw = (axes[0] == 3) ? w : 1;
    cvi_backend_tg_bf16_reduce_max_hw_kernel(ctx,
                                             layer_id,
                                             ga_input,
                                             ga_output,
                                             n,
...
void cvi_backend_tg_bf16_reduce_min_kernel(
  ...
  if (axes[0] == 1) {
    // along C axis
    cvi_backend_tg_bf16_reduce_min_chl_kernel(ctx,
                                              layer_id,
                                              ga_input,
                                              ga_output,
                                              n,
    ...
    int kh = (axes[0] == 2) ? h : 1;
    int kw = (axes[0] == 3) ? w : 1;
    cvi_backend_tg_bf16_reduce_min_hw_kernel(ctx,
                                             layer_id,
                                             ga_input,
                                             ga_output,
                                             n,
