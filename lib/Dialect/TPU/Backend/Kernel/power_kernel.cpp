...
#define DEBUG_TYPE "bmnet_bm1880v2_power"
...
static void one_step(
  ...
  if (fmt == CVK_FMT_I8) {
    if (power == 0) {
      ...
      tl_ifmap = ctx.lmem_alloc_tensor(ctx.shape_t4(input_n, input_c, input_h, input_w), fmt, /*eu_align=*/1);
...
void bmnet_power_fixed_forward_bmkernel(
  ...
  cvk_tl_t *tl_scale = ctx.lmem_alloc_tensor(ctx.shape_t4(1,
        NPU_NUM, 1, 1), (cvk_fmt_t)fmt, /*eu_align=*/1);
  // 2 means 16bits
  cvk_tl_t *tl_shift = ctx.lmem_alloc_tensor(ctx.shape_t4(2,
