...
void TgReluKernel::selectTilePolicy() {
  ...
  if (mode == PRELU) {
    ...
    ctx.tiling_packing(tiles, n, c, h, w, fmt, 4, slope_size,
...
void TgReluKernel::compute_leaky_relu_fixed(int32_t step_idx, int32_t flip) {
  auto tl_ifmap = get_input(step_idx, flip);
  auto tl_ofmap = get_output(step_idx, flip);
...
void TgReluKernel::compute(int32_t step_idx, int32_t flip) {
  ...
  case LEAKY_RELU:
    if (fmt == CVK_FMT_BF16) {
      ...
      compute_leaky_relu_fixed(step_idx, flip);
    }
    break;
  case PRELU:
    if (fmt == CVK_FMT_BF16) {
