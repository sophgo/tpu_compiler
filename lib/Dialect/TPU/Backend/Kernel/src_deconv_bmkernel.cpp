 ...
 *
 * File Name: src_deconv_bmkernel.cpp
 * Description:
 */
...
#define DEBUG_TYPE "bmkernel_deconv"
#define DEBUG_SPLIT "bmkernel_deconv_split"
...
void node_depthwise_deconv_fixed_forward_bmkernel(
  ...
  for (int oc_pos = 0; oc_pos < oc; oc_pos += oc_step) {
    ...
    if (do_chan_quan) {
      ...
      cvk_tl_shape_t bias_coeff_shape_ = ctx.shape_t4(2, cur_oc, 1, 1);
    ...
    for (int n_pos = 0; n_pos < input_n; n_pos += n_step) {
      ...
      for (int oh_pos = 0; oh_pos < output_h; oh_pos += oh_step) {
        ...
        cvk_tl_t *tl_ofmap =
            ctx.lmem_alloc_tensor(ctx.shape_t4(cur_n, cur_oc, cur_oh, output_w), CVK_FMT_I8, /*eu_align=*/1);
...

        {
          ...
          cvk_tl_t *tl_ifmap =
              ctx.lmem_alloc_tensor(ctx.shape_t4(cur_n, cur_ic, cur_ih, input_w), CVK_FMT_I8, /*eu_align=*/1);
          ...
          tl_weight = ctx.lmem_alloc_tensor(ctx.shape_t4(1, cur_ic, kh, kw), CVK_FMT_I8, /*eu_align=*/1);
...
void deconv_fixed_forward_bmkernel(
  ...
  tl_weight = ctx.lmem_alloc_tensor(ctx.shape_t4(1, oc_step, kh * kw, ic), CVK_FMT_I8,
                                    0);  // Not EU-aligned
      ...
      for (int n_pos = 0; n_pos < input_n; n_pos += n_step) {
        ...
        for (int oh_pos = 0; oh_pos < output_h; oh_pos += oh_step) {
          ...
          cvk_tl_t *tl_ifmap = ctx.lmem_alloc_tensor(
                ctx.shape_t4(cur_n, ic, cur_ih, input_w), CVK_FMT_I8, 1);
          ...
          uint64_t weight_offset = ga_weight + sizeof(uint8_t) * (oc_pos * ic * kh * kw +
                                                         ig * oc * ic * kh * kw);
          tl_weight->shape = ctx.shape_t4(1, cur_oc, kh * kw, ic);
          tl_weight->stride =
          ...
          // tl_reshape(tl_weight, shape_t4(cur_ic, cur_oc, kh, kw));
          tl_weight->shape = ctx.shape_t4(ic, cur_oc, kh, kw);
          tl_weight->stride =
              ctx.tl_default_stride(tl_weight->shape, CVK_FMT_I8, 0);  // Not EU-aligned
