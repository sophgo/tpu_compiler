...
void TgInt8PoolingKernel::init(uint32_t layer_id,
...
void TgInt8PoolingKernel::selectTilePolicy() {
  doTileForNormalCase();
}
    ...
    for (step_oh = stride_h > 15 ? 1 : oh; step_oh > 0; step_oh--) {
      for (step_c = std::min(c, MAX_CHANNEL); step_c > 0; step_c -= NPU_NUM) {
        ...
        LLVM_DEBUG(llvm::errs() << llvm::format(
            "try input shape:%d,%d,%d,%d output shape:%d,%d,%d,%d total lmem:%d\n",
            1, step_c, step_ih, step_iw, 1, step_c, step_oh, step_ow, total_lmem));
        if (total_lmem <= (uint32_t)LOCAL_MEM_SIZE) {
          LLVM_DEBUG(llvm::errs() << llvm::format(
              "input_shape(%d,%d,%d,%d), output_shape(%d,%d,%d,%d)\n"
              "kh:%d, kw:%d, sh:%d, sw:%d, pad(%d,%d,%d,%d), avg_pooling:%d, do_relu:%d\n"
              "step_c:%d, step_ih:%d, step_iw:%d, step_oh:%d, step_ow:%d\n",
              n, c, h, w, n, c, oh, ow, kh, kw, stride_h, stride_w, pad_t, pad_b, pad_l, pad_r,
              is_avg_pooling, do_relu, step_c, step_ih, step_iw, step_oh, step_ow));
...
do_tile:
  ...
  for (int c_pos = 0; c_pos < c; c_pos += step_c) {
    ...
    for (int oh_pos = 0; oh_pos < oh; oh_pos += step_oh) {
      ...
      int32_t cur_pad_t = (ih_top == 0) ?
                          (pad_t - oh_top * stride_h) : 0;
      int32_t cur_pad_b = (ih_bot == h) ?
                          ((oh_bot - 1) * stride_h + kh - pad_t - h) : 0;
      ...
      for (int ow_pos = 0; ow_pos < ow; ow_pos += step_ow) {
        ...
        int32_t cur_pad_l = (iw_left == 0) ?
                            (pad_l - ow_left * stride_w) : 0;
        int32_t cur_pad_r = (iw_right == w) ?
                            ((ow_right - 1) * stride_w + kw - pad_l - w) : 0;
        tile.n = 1;
        tile.c = cur_c;
        tile.h = cur_ih;
        tile.w = cur_iw;
        ...
        tile.input_offset = (c_pos * h * w + ih_top * w + iw_left) * sizeof(int8_t);
        tile.output_offset = (c_pos * oh * ow + oh_pos * ow + ow_pos) * sizeof(int8_t);
...
void cvi_backend_tg_fixed_avg_pooling_kernel(
    ...
    int n, int c, int h, int w, int kh, int kw,
    int pad_top, int pad_bot, int pad_left, int pad_right,
    int stride_h, int stride_w, bool do_relu,
    int rshift, int multiplier, bool ceil_mode) {
  ...
  kernel.init(
      layer_id, ga_input, ga_output, n, c, h, w,
      pad_top, pad_bot, pad_left, pad_right,
      kh, kw, stride_h, stride_w, true, do_relu,
