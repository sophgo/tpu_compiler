...
void cvi_backend_tg_bf16_gru_kernel(const CviBackendContext &ctx, uint32_t layer_id,
    ...
    tl_weight->shape = ctx.tl_shape_t4(input_size, 3 * hidden_size, 1, 1);
    tl_weight->stride = ctx.tl_default_stride(tl_weight->shape, CVK_FMT_BF16, /*eu_align=*/0);

    //Load recurrence (Rz + Rr + Rh)
    cvk_tl_shape_t reshape_recurrence_shape = ctx.tl_shape_t4(1, 3 * hidden_size, 1, hidden_size);
    ...
    //Reshape weight shape/stride to match convolution constraint
    //Just fit the constraint
    tl_recurrence->shape = ctx.tl_shape_t4(hidden_size, 3 * hidden_size, 1, 1);
    ...
    if(do_bias) {
        //set nstride
        cvk_tl_shape_t reshape_wtBias_shape = ctx.tl_shape_t4(2, 3 *hidden_size,1, 1);
        tl_wtBias =
    ...
    ASSERT(tl_output);
    //Allocate temp1 =  Xt*W
    cvk_tl_shape_t reshape_temp1_shape = ctx.tl_shape_t4(seq_len*batch_size, 3 * hidden_size,1,1);
    ...
    for(int i = 0; i < seq_len; i++){
        ...
        cvk_tl_shape_t reshape_h_mul_r_shape = ctx.tl_shape_t4(1, 3 * hidden_size,1, 1);
        cvk_tl_t *tl_h_mul_r =
        ...
        cvk_tl_shape_t reshape_zt_rt_lut_index_shape = ctx.tl_shape_t4(1, 2 * hidden_size,1, 1);
        ...
        //working
        cvk_tl_shape_t reshape_temp4_shape = ctx.tl_shape_t4(2,2 * hidden_size,1,1);
        cvk_tl_t *tl_temp4 =
                ctx.lmem_alloc_tensor(reshape_temp4_shape, CVK_FMT_BF16, 1); //weight EU_ALIGN = False
        ASSERT(tl_temp4);

        //output
        cvk_tl_shape_t reshape_zt_ht_shape = ctx.tl_shape_t4(1,2 * hidden_size,1,1);
        cvk_tl_t *tl_zt_rt =
        ...
        //output
        cvk_tl_shape_t reshape_ht_shape = ctx.tl_shape_t4(1,hidden_size,1,1);
        cvk_tl_t *tl_ht =
