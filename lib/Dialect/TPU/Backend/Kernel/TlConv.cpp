...
static void conv_la_oc_step(const CviBackendContext &ctx, uint32_t layer_id,
  ...
  tl_ifmap = ctx.lmem_alloc_tensor(ctx.tl_shape_t4(n, ic, ih, iw), CVK_FMT_I8, /*eu_align=*/1);
  ...
  tl_perchannel->shape = ctx.tl_shape_t4(1, oc, 1, perchannel_size);
  ...
  for (uint32_t n_pos = 0; n_pos < n; n_pos++) {
    ...
    tl_ifmap_step.shape = ctx.tl_shape_t4(1, ic, ih, iw);
    ...
    for (uint32_t oc_pos = 0; oc_pos < oc; oc_pos += oc_step) {
      ...
      tl_tmp.shape = ctx.tl_shape_t4(1, cur_oc, kh * kw, ic / g);
      ...
      //tl_perchannel->shape = ctx.tl_shape_t4(1, cur_oc, 1, 1);
      //tl_perchannel->stride = ctx.tl_default_stride(tl_perchannel->shape, CVK_FMT_I8, /*eu_aign=*/0);
      ...
      tl_perchannel_oc_pos.shape = ctx.tl_shape_t4(1, cur_oc, 1, 1); // 1? not 9 or 5? very strange
      ...
      // bmk does not keep eu-align info, user need to update stride if shape changed
      //tl_ofmap->shape = ctx.tl_shape_t4(n, cur_oc, oh, ow);
      ...
      tl_ofmap_oc_pos.shape = ctx.tl_shape_t4(1, cur_oc, oh, ow);
      ...
      if  (!is_dw) {
        ...
        // bmk does not keep eu-align info, user need to update stride if shape changed
        tl_filter[flip]->shape = ctx.tl_shape_t4(ic / g, cur_oc, kh, kw);
        ...
        tl_ifmap_oc_pos.shape = ctx.tl_shape_t4(1, cur_oc, ih, iw);
        tl_ifmap_oc_pos.stride = ctx.tl_default_stride(tl_ifmap_oc_pos.shape, CVK_FMT_I8, /*eu_align=*/1);

        // filter shape for tiu != shape for load
        // bmk does not keep eu-align info, user need to update stride if shape changed
        tl_filter[flip]->shape = ctx.tl_shape_t4(1, cur_oc, kh, kw);
...
static void conv_lw_oc_step(const CviBackendContext &ctx, uint32_t layer_id,
  ...
  tl_ifmap_step.fmt = CVK_FMT_I8;
  tl_ifmap_step.shape = ctx.tl_shape_t4(n, ic, ih, iw);
  tl_ifmap_step.stride = ctx.tl_default_stride(tl_ifmap_step.shape, CVK_FMT_I8, /*eu_align=*/1);
  for (uint32_t oc_pos = 0; oc_pos < oc; oc_pos += oc_step) {
    uint32_t cur_oc = std::min(oc - oc_pos, oc_step);
    cvk_tl_t tl_filter;
    ...
    tl_filter.shape = ctx.tl_shape_t4(1, cur_oc, kh * kw, ic / g);
    ...
    tl_perchannel.shape = ctx.tl_shape_t4(1, cur_oc, 1, perchannel_size);
    tl_perchannel.stride = ctx.tl_default_stride(tl_perchannel.shape, CVK_FMT_I8, /*eu_aign=*/0);
    ...
    if (!is_dw) {
      ...
      tl_ifmap_oc_pos.fmt = CVK_FMT_I8;
      tl_ifmap_oc_pos.shape = ctx.tl_shape_t4(n, cur_oc, ih, iw);
      tl_ifmap_oc_pos.stride = ctx.tl_default_stride(tl_ifmap_oc_pos.shape, CVK_FMT_I8, /*eu_align=*/1);
      tl_ifmap_oc_pos.stride.n = tl_ifmap_oc_pos.stride.c * ceiling_func(oc, NPU_NUM);
      // tl_filter shape for tiu != shape for load
      tl_filter.shape = ctx.tl_shape_t4(1, cur_oc, kh, kw);
    ...
    if (do_leaky_relu) {
      ...
      for(int n_step = 0; n_step < (int)n; n_step++) {
        ...
        while (relu_pos < relu_size) {
          ...
          tl_relu.shape = ctx.tl_shape_t4(1, cur_oc, 1, relu_cur);
    ...
    if (do_store) {
      ...
      if (first) {
        ...
        tl_ofmap_oc_pos_flip_back.shape = ctx.tl_shape_t4(n, cur_oc_flip_back, oh, ow);
...
void cvi_backend_tl_conv(
  ...
  if (group == input_c && group == output_c && group != 1) {
    ...
    cvk_tl_shape_t tl_weight_shape = ctx.tl_shape_t4(input_c, output_c, kh, kw);
    tl_weight.start_address = la_weight;
    tl_weight.fmt = CVK_FMT_I8;
    tl_weight.shape = tl_weight_shape;
  ...
  tl_chl_quan_param.shape = ctx.tl_shape_t4(1, output_c, 1, 1);
  ...
  if (group == 1) {
    ...
    for (int32_t oc_pos = 0; oc_pos < output_c; oc_pos += oc_step) {
      ...
      cvk_tl_shape_t output_shape = ctx.tl_shape_t4(input_n, cur_oc, output_h, output_w);
      ...
      // filter shape for tiu != shape for load
      cvk_tl_shape_t weight_shape = ctx.tl_shape_t4(input_c, cur_oc, kh, kw);
    ...
    for (int ig = 0; ig < group; ig++) {
      ...
      for (int nidx = 0; nidx < input_n; nidx++) {
        ...
        // input
        cvk_tl_shape_t input_shape = ctx.tl_shape_t4(1, ic, input_h, input_w);
        cvk_tl_t input;
        ...
        // weight
        cvk_tl_shape_t weight_shape = ctx.tl_shape_t4(ic,oc,kh,kw);
        cvk_tl_t weight;
        ...
        // output
        cvk_tl_shape_t output_shape = ctx.tl_shape_t4(1,oc,output_h,output_w);
        cvk_tl_t output;
        ...
        tl_chl_quan_param.shape = ctx.tl_shape_t4(1,oc,1,1);
        tl_chl_quan_param.stride =
            ctx.tl_default_stride(tl_chl_quan_param.shape, CVK_FMT_I8, 0);
