...
void cvi_backend_tl_prelu(const CviBackendContext &ctx, uint32_t layer_id,
  ...
  tl_slope->shape = ctx.tl_shape_t4(1, c, 1, 1);
  tl_slope->stride = ctx.tl_default_stride(tl_slope->shape, CVK_FMT_I8, /*eu_align=*/1);
...
void cvi_backend_tl_bf16_prelu(const CviBackendContext &ctx,
                              uint32_t layer_id,
                              laddr_t la_input,
                              laddr_t la_output,
                              laddr_t la_slope,
  ...
  tl_input->shape = ctx.tl_shape_t4(n, c, h, w);
  tl_input->stride = ctx.tl_default_stride(tl_input->shape, CVK_FMT_BF16, /*eu_align=*/1);

  tl_output->start_address = la_output;
  tl_output->fmt = CVK_FMT_BF16;
  tl_output->shape = ctx.tl_shape_t4(n, c, h, w);
  tl_output->stride = ctx.tl_default_stride(tl_output->shape, CVK_FMT_BF16, /*eu_align=*/1);

  tl_slope->start_address = la_slope;
  tl_slope->fmt = CVK_FMT_BF16;
  tl_slope->shape = ctx.tl_shape_t4(1, c, 1, 1);
  tl_slope->stride = ctx.tl_default_stride(tl_slope->shape, CVK_FMT_BF16, /*eu_align=*/1);
