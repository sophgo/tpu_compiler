...
class CviBackendContext : public BM188xBackendContext {
 public:
  CviBackendContext(const char *runchip, int nodechip_num, std::vector<int8_t> &weight);
  ~CviBackendContext() override;
  ...
  void parallel_enable() const override {
    if (cvk_ctx_)
      cvk_ctx_->ops->parallel_enable(cvk_ctx_);
  }

  void parallel_disable() const override {
    if (cvk_ctx_)
      cvk_ctx_->ops->parallel_disable(cvk_ctx_);
  }

  void set_layer_id(uint16_t layer_id) const override {
    if (cvk_ctx_)
      cvk_ctx_->ops->set_layer_id(cvk_ctx_, layer_id);
  }

  void tdma_l2l_tensor_copy(
      const cvk_tdma_l2l_tensor_copy_param_t *param ) const {
    if (cvk_ctx_)
  ...
  void tdma_l2l_tensor_lrn_shift(
      const cvk_tdma_l2l_tensor_lrn_shift_param_t *param) const {
    if (cvk_ctx_)
      cvk_ctx_->ops->tdma_l2l_tensor_lrn_shift(cvk_ctx_, param);
  }

  void _tdma_l2g_tensor_copy(
      const cvk_tdma_l2g_tensor_copy_param_t *param) const {
    if (cvk_ctx_ && is_one_bf16(param->src->fmt, param->dst->fmt))
      cvk_ctx_->ops->tdma_l2g_bf16_tensor_copy(cvk_ctx_, param);
    else if (cvk_ctx_)
      cvk_ctx_->ops->tdma_l2g_tensor_copy(cvk_ctx_, param);
  }

  void _tdma_l2l_tensor_copy(
      const cvk_tdma_l2l_tensor_copy_param_t *param) const {
    if (cvk_ctx_ && is_one_bf16(param->src->fmt, param->dst->fmt))
      cvk_ctx_->ops->tdma_l2l_bf16_tensor_copy(cvk_ctx_, param);
    else if (cvk_ctx_)
  ...
  void _tdma_l2g_tensor_copy_nc_transposed(
      const cvk_tdma_l2g_tensor_copy_nc_transposed_param_t *param) const {
    if (cvk_ctx_ && is_one_bf16(param->src->fmt, param->dst->fmt))
  ...
  void _tdma_g2l_tensor_copy_nc_transposed(
      const cvk_tdma_g2l_tensor_copy_nc_transposed_param_t *param) const {
    if (cvk_ctx_ && is_one_bf16(param->src->fmt, param->dst->fmt))
  ...
  void _tdma_g2l_tensor_copy(
      const cvk_tdma_g2l_tensor_copy_param_t *param) const {
    if (cvk_ctx_ && is_one_bf16(param->src->fmt, param->dst->fmt))
  ...
  void tdma_l2g_tensor_copy_cw_transposed(
      const cvk_tdma_l2g_tensor_copy_cw_transposed_param_t *param) const {
    if (cvk_ctx_)
  ...
  void tdma_l2g_general_copy(
      const cvk_tdma_l2g_general_copy_param_t *param) const {
    if (cvk_ctx_)
      cvk_ctx_->ops->tdma_l2g_general_copy(cvk_ctx_, param);
  }

  void tdma_l2g_bf16_general_copy(
      const cvk_tdma_l2g_bf16_general_copy_param_t *param) const {
    if (cvk_ctx_)
  ...
  void tdma_g2l_tensor_copy_chw_rotated(
      const cvk_tdma_g2l_tensor_copy_chw_rotated_param_t *param) const {
    if (cvk_ctx_)
      cvk_ctx_->ops->tdma_g2l_tensor_copy_chw_rotated(cvk_ctx_, param);
  }

  void tdma_g2l_tensor_copy_decompressed(
      const cvk_tdma_g2l_tensor_copy_decompressed_param_t *param) const {
    if (cvk_ctx_)
      cvk_ctx_->ops->tdma_g2l_tensor_copy_decompressed(cvk_ctx_, param);
  }

  void tdma_tg2l_tensor_fill_constant(
      const cvk_tdma_g2l_tensor_fill_constant_param_t *param) const {
    if (cvk_ctx_)
      cvk_ctx_->ops->tdma_g2l_tensor_fill_constant(cvk_ctx_, param);
  }

  void tdma_tg2l_bf16_tensor_fill_constant(
      const cvk_tdma_g2l_tensor_fill_constant_param_t *param) const {
    if (cvk_ctx_)
  ...
  void tdma_g2l_matrix_copy_decompressed(
      const cvk_tdma_g2l_matrix_copy_decompressed_param_t *param) const {
    if (cvk_ctx_)
      cvk_ctx_->ops->tdma_g2l_matrix_copy_decompressed(cvk_ctx_, param);
  }

  void tdma_g2l_matrix_copy(
      const cvk_tdma_g2l_matrix_copy_param_t *param) const {
    if (cvk_ctx_)
  ...
  void tiu_and_int16(const cvk_tiu_and_int16_param_t *param) {
    if (cvk_ctx_)
  ...
  void tiu_bf16_lookup_interp_table(
      const cvk_tiu_bf16_lookup_interp_table_param_t *param) const {
    if (cvk_ctx_)
  ...
  cvk_tg_stride_t tg_default_stride(
      cvk_tg_shape_t shape, cvk_fmt_t fmt) const {
    if (cvk_ctx_)
  ...
  cvk_ml_stride_t ml_default_stride(
      cvk_ml_shape_t shape, cvk_fmt_t fmt, int eu_align) const {
    if (cvk_ctx_)
  ...
  cvk_ml_t *lmem_alloc_ps32_matrix(
      cvk_ml_shape_t shape, cvk_fmt_t fmt, int eu_align) const {
    if (cvk_ctx_)
      return cvk_ctx_->ops->lmem_alloc_ps32_matrix(cvk_ctx_, shape, fmt,
                                                   eu_align);
  ...
  void lmem_init_tensor(cvk_tl_t *tl, cvk_tl_shape_t shape, cvk_fmt_t fmt,
      int eu_align) const {
    if (cvk_ctx_)
  ...
  void lmem_init_matrix(
      cvk_ml_t *ml, cvk_ml_shape_t shape, cvk_fmt_t fmt, int eu_align) const {
    if (cvk_ctx_)
  ...
  uint32_t lmem_tensor_to_size(
      cvk_tl_shape_t shape, cvk_fmt_t fmt, int eu_align) const {
    if (cvk_ctx_)
      return cvk_ctx_->ops->lmem_tensor_to_size(cvk_ctx_, shape, fmt,
                                                eu_align);
    return (uint32_t)-1;
  }
  ...
  uint32_t lmem_matrix_to_size(
      cvk_ml_shape_t shape, cvk_fmt_t fmt, int eu_align) const {
    if (cvk_ctx_)
      return cvk_ctx_->ops->lmem_matrix_to_size(cvk_ctx_, shape, fmt,
                                                eu_align);
  ...
  uint32_t lmem_ps32_matrix_to_size(
      cvk_ml_shape_t shape, cvk_fmt_t fmt, int eu_align) const {
    if (cvk_ctx_)
      return cvk_ctx_->ops->lmem_ps32_matrix_to_size(cvk_ctx_, shape, fmt,
                                                     eu_align);
  ...
  void gmem_init_tensor(cvk_tg_t *tg, cvk_tg_shape_t shape, cvk_fmt_t fmt)
      const {
    if (cvk_ctx_)
  ...
  void tiu_depthwise_convolution(
      const cvk_tiu_depthwise_convolution_param_t *param) const {
    if (cvk_ctx_)
      cvk_ctx_->ops->tiu_depthwise_convolution(cvk_ctx_, param);
  }

  void tiu_matrix_multiplication_qm(
      const cvk_tiu_matrix_multiplication_qm_param_t *param) const {
    if (cvk_ctx_)
  ...
  inline cvk_tl_t convert_dma_quan_param(cvk_tl_t *tl_quan_param,
                                         bool do_bias) const {
    // channel quant param load shape != tiu shape
    uint32_t param_size = this->chan_quan_param_size(do_bias);

    cvk_tl_t tl_quan_param_load;
    tl_quan_param_load.start_address = tl_quan_param->start_address;
    tl_quan_param_load.fmt = tl_quan_param->fmt;
    tl_quan_param_load.shape = {1, tl_quan_param->shape.c, 1, param_size};
    //this->tl_default_stride(tl_quan_param_load.shape, CVK_FMT_I8, /*eu_align=*/0);

    return tl_quan_param_load;
  }
  ...
  //
  uint32_t tl_shape_to_size(cvk_tl_shape_t shape, bool aligned, cvk_fmt_t fmt) const;
  ...
  inline cvk_tl_shape_t shape_t4(int n, int c, int h, int w) const {
    return {static_cast<uint32_t>(n), static_cast<uint32_t>(c), static_cast<uint32_t>(h), static_cast<uint32_t>(w)};
  }
  ...
  inline cvk_tl_t tl_prealloc(uint32_t la_addr, cvk_tl_shape_t shape,
                              cvk_fmt_t fmt) const {
    cvk_tl_t tl_data = {0};
    tl_data.start_address = la_addr;
    tl_data.fmt = fmt;
  ...
  // TDMA legacy API
  //
  void tdma_load_stride(cvk_tl_t *tlp, uint64_t ga_src,
                        cvk_tg_stride_t ts_stride,
                        uint8_t do_transpose = 0) const ;

  void tdma_load_stride_bf16(cvk_tl_t *tlp, uint64_t ga_src,
                             cvk_tg_stride_t ts_stride,
                             uint8_t do_transpose = 0) const;

  void tdma_load(cvk_tl_t *tlp, uint64_t ga_src,
                 uint8_t do_transpose = 0) const;

  void tdma_load_bf16(cvk_tl_t *tlp, uint64_t ga_src,
                      uint8_t do_transpose = 0) const;

  void tdma_store_stride(cvk_tl_t *tlp, uint64_t ga_dst,
                         cvk_tg_stride_t ts_stride,
                         uint8_t do_transpose = 0) const;

  void tdma_store_stride_bf16(cvk_tl_t *tlp, uint64_t ga_dst,
                              cvk_tg_stride_t ts_stride,
                              uint8_t do_transpose = 0) const;

  void tdma_store(cvk_tl_t *tlp, uint64_t ga_dst,
                  uint8_t do_transpose = 0) const;

  void tdma_store_bf16(cvk_tl_t *tlp, uint64_t ga_dst,
                       uint8_t do_transpose = 0) const;
  ...
  // matrix format
  void tdma_load_stride(cvk_ml_t *tlp, uint64_t ga_src,
                        cvk_mg_stride_t ts_stride,
                        uint8_t do_transpose = 0) const;

  void tdma_load_stride_bf16(cvk_ml_t *tlp, uint64_t ga_src,
                             cvk_mg_stride_t ts_stride,
                             uint8_t do_transpose = 0) const;

  void tdma_load(cvk_ml_t *tlp, uint64_t ga_src,
                 uint8_t do_transpose = 0) const;
  void tdma_load_bf16(cvk_ml_t *tlp, uint64_t ga_src,
                      uint8_t do_transpose = 0) const;

  void tdma_store_stride(cvk_ml_t *tlp, uint64_t ga_dst,
                         cvk_mg_stride_t ts_stride,
                         uint8_t do_transpose = 0) const;

  void tdma_store_stride_bf16(cvk_ml_t *tlp, uint64_t ga_dst,
                              cvk_mg_stride_t ts_stride,
                              uint8_t do_transpose = 0) const;

  void tdma_store(cvk_ml_t *tlp, uint64_t ga_dst,
                  uint8_t do_transpose = 0) const;

  void tdma_store_bf16(cvk_ml_t *tlp, uint64_t ga_dst,
                       uint8_t do_transpose = 0) const;
