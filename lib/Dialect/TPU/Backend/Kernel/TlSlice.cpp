...
void cvi_backend_tl_slice(
  ...
  for (int i = 0; i < input_dim[0]; i++) {
    auto n_start_laddr = la_input +
                         ctx.lmem_tensor_to_size(1, input_dim[1],
                                                input_dim[2], input_dim[3]) * i;

    auto cur_lane_size = ctx.lmem_tensor_to_size(1, 1,
...
void cvi_backend_tl_bf16_slice(
  ...
  for (int i = 0; i < input_dim[0]; i++) {
    auto n_start_laddr = la_input +
                         ctx.lmem_tensor_to_size(1, input_dim[1], input_dim[2],
                                            input_dim[3], CVK_FMT_BF16) * i;
