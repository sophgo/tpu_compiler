...
void cvi_backend_tl_pooling(
  ...
  LLVM_DEBUG(llvm::errs() << llvm::format(
                  ...
                  ifmap_laddr, ofmap_laddr, input_n, input_c, input_h, input_w, output_n, output_c,
                  output_h, output_w, kw, kh, stride_h, stride_w, pad_h_top, pad_h_bottom,
                  pad_w_left, pad_w_right, is_avg_pooling, rshift,
                  m_i8));

  // Input feature map with assigned local memory address
  cvk_tl_shape_t ifmap_shape = {static_cast<uint32_t>(input_n), static_cast<uint32_t>(input_c),
  ...
  cvk_tl_shape_t ofmap_shape = {
      static_cast<uint32_t>(output_n), static_cast<uint32_t>(output_c), static_cast<uint32_t>(output_h),
      static_cast<uint32_t>(output_w)};
  cvk_tl_t ofmap;
  ofmap.start_address = ofmap_laddr;
  ofmap.fmt = CVK_FMT_I8;
  ofmap.shape = ofmap_shape;
  ofmap.stride = ctx.tl_default_stride(ofmap_shape, CVK_FMT_I8, 1);  // EU-aligned
...
void cvi_backend_tl_bf16_pooling(
  ...
  LLVM_DEBUG(llvm::errs() << llvm::format(
                  ...
                  ifmap_laddr, ofmap_laddr, input_n, input_c, input_h, input_w, output_n, output_c,
                  output_h, output_w, kw, kh, stride_h, stride_w, pad_h_top, pad_h_bottom,
                  pad_w_left, pad_w_right, is_avg_pooling));
  ...
  ifmap.shape = ifmap_shape;
  ifmap.stride = ctx.tl_default_stride(ifmap_shape, CVK_FMT_BF16, 1);  // EU-aligned
  ...
  ofmap.shape = ofmap_shape;
  ofmap.stride = ctx.tl_default_stride(ofmap_shape, CVK_FMT_BF16, 1);  // EU-aligned
