    ...
    for (int32_t oh_step = std::min(oh, MAX_HEIGHT); oh_step > 0; --oh_step) {
      ...
      for (int32_t slice_oc = 0; slice_oc < num_oc_step; ++slice_oc) {
        ...
        if (do_chl_quan) {
          if (do_bias) {
            cvk_tl_shape_t coeff_shape_9byte =
                ctx_.shape_t4(1, oc_step, 1, 9);
            coeff_oc_step_size += ctx_.lmem_tensor_to_size(coeff_shape_9byte,
                                      args_.tiu_fmt, /*eu_align=*/0);
          } else {
            cvk_tl_shape_t coeff_shape_5byte = ctx_.shape_t4(1, oc_step, 1, 5);
        ...
        coeff_oc_step_size += ctx_.lmem_tensor_to_size(
                                  ctx_.shape_t4(ic_step, oc_step, kh, kw),
                                  args_.tiu_fmt, /*eu_align=*/0);
  ...
  for (slices_.w = 1; slices_.w <= ow; ++slices_.w) {
    ...
    for (slices_.h = 1; slices_.h <= oh; ++slices_.h) {
      ...
      for (slices_.n = 1; slices_.n <= n; ++slices_.n) {
        ...
        uint32_t ifmap_size = ctx_.lmem_tensor_to_size(
                            ctx_.shape_t4(n_step, oc_step, ih_step, iw_step),
                            args_.tiu_fmt, /*eu_align=*/1);
        total_needed += ifmap_size;
...
void Conv::dwConv() {
  ...
  //
  tl_weight[0] =
      ctx_.lmem_alloc_tensor(ctx_.shape_t4(1, oc_step, kh, kw), args_.tiu_fmt, /*eu_align=*/0);
  tl_weight[1] =
      ctx_.lmem_alloc_tensor(ctx_.shape_t4(1, oc_step, kh, kw), args_.tiu_fmt, /*eu_align=*/0);
  tl_ifmap[0] = ctx_.lmem_alloc_tensor(ctx_.shape_t4(n_step, oc_step, ih_step, iw_step), args_.tiu_fmt,
                                      /*eu_align=*/1);
  ...
  if (do_chl_quan) {
    ...
    if (do_bias) {
      cvk_tl_shape_t coeff_shape_9byte = ctx_.shape_t4(1, oc_step, 1, 9);
  ...
  for (int ig = 0; ig < 1; ++ig) {
    ...
    for (int oc_pos = 0; oc_pos < oc; oc_pos += oc_step) {
      ...
      if (do_chl_quan) {
        ...
        if (do_bias) {
          tl_chl_quan[coeff_flip]->shape = ctx_.shape_t4(1, cur_oc, 1, 9);
          ...
          tl_chl_quan[coeff_flip]->shape = ctx_.shape_t4(1, cur_oc, 1, 5);
          tl_chl_quan[coeff_flip]->stride =
              ctx_.tl_default_stride(tl_chl_quan[coeff_flip]->shape, args_.tiu_fmt, /*eu_aign=*/0);
      ...
      // bmk does not keep eu-align info, user need to update stride if shape changed
      tl_weight[coeff_flip]->shape = ctx_.shape_t4(1, cur_oc, kh, kw);
      ...
      uint64_t weight_offset = ga_weight + oc_pos * weight_gstride.c;
      {
        ...
        tl_tmp.shape = ctx_.shape_t4(1, cur_oc, kh * kw, 1);
//        tl_tmp.shape = ctx_.shape_t4(1, cur_oc, kh * kw, ic_step);
      ...
      for (int n_pos = 0; n_pos < input_n; n_pos += n_step) {
        ...
        for (int oh_pos = 0; oh_pos < oh; oh_pos += oh_step) {
          ...
          for (int ow_pos = 0; ow_pos < ow; ow_pos += ow_step) {
            ...
            // bmk does not keep eu-align info, user need to update stride if shape changed
            tl_ifmap[flip]->shape = ctx_.shape_t4(cur_n, cur_oc, cur_ih, cur_iw);
...
bool Conv::canNoTile() {
  ...
  // Add weight size
  coeff_size += ctx_.lmem_tensor_to_size(ctx_.shape_t4(ic, oc, kh, kw),
                                         args_.tiu_fmt,/*eu_align=*/0);

  uint32_t total_needed = 0;

  uint32_t ofmap_size =
      ctx_.lmem_tensor_to_size(ctx_.shape_t4(n, oc, oh, ow), args_.tiu_fmt,
                              /*eu_align=*/1);
  total_needed += ofmap_size;

  uint32_t ifmap_size =
      ctx_.lmem_tensor_to_size(ctx_.shape_t4(n, ic, ih, iw), args_.tiu_fmt,
                              /*eu_align=*/1);
  total_needed += ifmap_size;
