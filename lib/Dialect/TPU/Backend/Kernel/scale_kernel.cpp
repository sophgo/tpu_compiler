...
#define DEBUG_TYPE "bmnet_bm1880v2_bmkernel_scale"
#define DEBUG_SPLIT "bmnet_bm1880v2_bmkernel_scale_split"
...
#define RELU (0)
  ...
  */
  ...
  if (is_scale_const) {
    LLVM_DEBUG(llvm::errs() << llvm::format("bmnet_quantization_fixed_forward_bmkernel:\n"
  ...
  for (int nidx = 0, nstart = 0; nidx < nsecs; nidx++) {
    ...
    for (int hidx = 0, hstart = 0; hidx < hsecs; hidx++) {
      ...
      if (qmode == CviBackendContext::QuantizeMode::INT8_PER_LAYER) {
        ...
        tl_bias->shape = ctx.shape_t4(1, input_c, 1, 1);
