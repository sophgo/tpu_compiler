...
#define RELU (0)
  ...
  */
  ...
  uint32_t scale_size = __get_lmem_usage(ctx, 1, input_c, 1, 1);
  laddr_t la_perchannel = la_working + scale_size;
  int perchannel_size = do_bias ? 9 : 5;

  cvk_tl_t tl_perchannel;
  tl_perchannel.start_address = la_perchannel;
  tl_perchannel.fmt = CVK_FMT_I8;
  tl_perchannel.shape = ctx.shape_t4(1, input_c, 1, perchannel_size);//when applying this, this is for move, so set w to perchannel_size
  ...
  ofmap.stride = ctx.tl_default_stride(ifmap_shape, CVK_FMT_I8, 1);  // EU-aligned
    ...
    if (qmode == CviBackendContext::QuantizeMode::INT8_PER_LAYER) {
      ...
      tl_perchannel.shape = ctx.shape_t4(1, input_c, 1, 1);
