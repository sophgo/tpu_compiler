...
void cvi_backend_tl_bf16_layernorm(const CviBackendContext &ctx,
  ...
  auto lshape = ctx.tl_shape_t4(n, c, h, w);
  auto table_shape = ctx.lut_table_shape(fmt);
  auto mean_shape = ctx.tl_shape_t4(n, c, 1, 1);
  auto mean_size = ctx.lmem_tensor_to_size(mean_shape, fmt, 1);

  ctx.lmem_init_tensor(&tl_input, lshape, fmt, 1);
  tl_input.start_address = la_input;
  ctx.lmem_init_tensor(&tl_output, lshape, fmt, 1);
  tl_output.start_address = la_output;
  ctx.lmem_init_tensor(&tl_lut, table_shape, fmt, 1);
  ...
  ctx.lmem_init_tensor(&tl_square, lshape, fmt, 1);
  tl_square.start_address = la_working;
  ctx.lmem_init_tensor(&tl_mean, mean_shape, fmt, 1);
  tl_mean.start_address =
      tl_square.start_address + ctx.lmem_tensor_to_size(lshape, fmt, 1);
  ctx.lmem_init_tensor(&tl_buff, mean_shape, fmt, 1);
  tl_buff.start_address = tl_mean.start_address + mean_size;
  ctx.lmem_init_tensor(&tl_var, mean_shape, fmt, 1);
  tl_var.start_address = tl_buff.start_address + mean_size;
  ctx.lmem_init_tensor(&tl_scale, lshape, fmt, 1);
  ctx.lmem_init_tensor(&tl_bias, lshape, fmt, 1);
