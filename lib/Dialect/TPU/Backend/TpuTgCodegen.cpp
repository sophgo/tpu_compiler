...
LogicalResult tpu::TG_WhereOp::codegen(void *ctx) {
  LLVM_DEBUG(llvm::errs() << "TG_codegen: " << getOperationName()
               << " [" << getOpName() << "]\n";);
  CviBackendContext *backend_ctx = (CviBackendContext *)ctx;
  Operation *op = this->getOperation();

  RankedTensorType in_type = this->getOperand(0).getType().cast<RankedTensorType>();
  cvk_fmt_t tType;
  ...
  gaddr_t ga_input = getPreviousOpAddress(op);
  gaddr_t ga_condition = getWeightOpAddress(condition().getDefiningOp());
  gaddr_t ga_output = getOpAddress(op);
  assert(isTensorNone(x()) && "only support masked_fill mode");

  int layer_id = getOpLayerId(op);
  std::vector<int64_t> _input_shape = getTensorShape(input());
  std::vector<int64_t> _condition_shape = getTensorShape(condition());
  assert(_input_shape.size() == _condition_shape.size() &&
...
#define RANKS (4)
  ...
  for (int i = 0; i < (int)input_shape.size(); i++) {
      input_shape[i + shift_dim] = _input_shape[i];
      condition_shape[i + shift_dim] = _condition_shape[i];
  }
  ...
  auto fill_constant  = this->fill_constant().convertToFloat();

  cvi_backend_tg_where_kernel(*backend_ctx,
                                    layer_id,
                                    ga_input,
                                    ga_condition,
                                    ga_output,
                                    input_shape.data(),
                                    condition_shape.data(),
                                    rshift,
                                    multiplier,
                                    fill_constant,
                                    tType
