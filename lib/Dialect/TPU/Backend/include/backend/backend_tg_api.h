...
void cvi_backend_tg_fixed_fc_kernel(
    ...
    std::vector<uint8_t> *compr_weight, int batch_high = 1, int batch_low = 1,
...
void cvi_backend_tg_bf16_conv_kernel(
    ...
    int load_cmpr_act, bool do_cmpr_wgt, int store_cmpr_act_c_step,
    int load_cmpr_act_c_step, int store_cmpr_act_h_step,
    int load_cmpr_act_h_step, bool do_quant = false, gaddr_t ga_scale = 0,
...
void cvi_backend_tg_bf16_fc_kernel(
    ...
    std::vector<uint8_t> *compr_weight, int batch_high = 1, int batch_low = 1,
    bool lstride = false, bool rstride = false, bool ostride = false,
    bool do_quant_bf16 = 0, gaddr_t ga_scale = 0, gaddr_t ga_zeropoint = 0);
...
void cvi_backend_tg_bf16_lstm_kernel(
    const CviBackendContext &ctx, uint32_t layer_id, gaddr_t ga_input,
    gaddr_t ga_recurrence, gaddr_t ga_bias, gaddr_t ga_initial_h,
    gaddr_t ga_inital_c, gaddr_t ga_cont, gaddr_t ga_sigmoid_table_data_lut,
    gaddr_t ga_sigmoid_slope_table_data_lut, gaddr_t ga_tanh_table_data_lut,
