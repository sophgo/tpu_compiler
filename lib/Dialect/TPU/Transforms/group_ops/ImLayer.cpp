...
void ImLayer::add_in_tensor(int n, int c, int h, int w, int unit_size, std::string& storage , const std::string& name,
                            tensor_type_t type) {
  std::shared_ptr<Tensor> tensor = Tensor::register_tensor(n, c, h, w, unit_size, storage, name, type);
  in_tensors.push_back(tensor);
}

void ImLayer::add_in_tensor(ShapedType* shape, const std::string& name, tensor_type_t type) {
  std::shared_ptr<Tensor> tensor = Tensor::register_tensor(shape, name, type);
  in_tensors.push_back(tensor);
}
...
void ImLayer::add_in_tensor(Value * v, tensor_type_t type) {
  ...
  if (def_op && !isa<tpu::NoneOp>(def_op) && !isa<tpu::WeightFileOp>(def_op)
    ...
    if (auto load_op = dyn_cast<tpu::LoadWeightOp>(def_op)) {
      ...
      std::shared_ptr<Tensor> tensor = Tensor::register_tensor(&shape, name, TENSOR_COEFF);
      in_tensors.push_back(tensor);
    } else {
...
void ImLayer::add_out_tensor(Value * v, tensor_type_t type) {
  ...
  if (!isa<tpu::NoneOp>(def_op) && !isa<tpu::WeightFileOp>(def_op)) {
    if (auto load_op = dyn_cast<tpu::LoadWeightOp>(def_op)) {
      ...
      std::shared_ptr<Tensor> tensor = Tensor::register_tensor(&shape, "return", TENSOR_NEURON);
      out_tensors.push_back(tensor);
    } else {
      std::string name = mlir::getOpName(def_op);
      std::shared_ptr<Tensor> tensor = Tensor::register_tensor(&shape, name, type);
...
void ImLayer::add_imm_tensor(const std::shared_ptr<Tensor> associcate, int count, const std::string& name) {
  std::shared_ptr<Tensor> tensor = Tensor::register_imm_tensor(associcate, count, name_ + "_imm");
...
static std::string getOperandStorage(Operation *p) {
...
ImConv::ImConv(Operation* p) : ImLayer(IR_CONVOLUTION, p, true), conv1x1_to_fc(false) {
  ...
  std::string storage = getOperandStorage(weightOp);
  if (is_dw) {
  ...
  std::string bias_storage = getOperandStorage(load_bias);
...
ImInnerproduct::ImInnerproduct(Operation* op) : ImLayer(IR_INNERPRODUCT, op) {
  ...
  if (!isa<tpu::NoneOp>(op->getOperand(2)->getDefiningOp())) {
    ...
    int bias_usize = getOperandStorageSize(load_bias);
    std::string storage = getOperandStorage(load_bias);
...
ImActivation::ImActivation(Operation* op) : ImLayer(IR_ACTIVATION, op, true) {
  ...
  std::string storage = getOperandStorage(load_y_table);
  ...
  if (0) {
    ...
    std::string storage = getOperandStorage(load_m_table);
...
ImLrn::ImLrn(Operation *op): ImLayer(IR_LRN, op, true) {
  ...
  std::string storage = getOperandStorage(load_sqr);
