...
// reshape: 1x32x32x32 -> 1x1x32x32x32
//   reduce_sum: 1x1x32x32x32 -> 1x32x32x32
struct TpuReshapeReduceSumPattern : public RewritePattern {
  TpuReshapeReduceSumPattern(MLIRContext *context)
      : RewritePattern(tpu::ReshapeOp::getOperationName(), 1, context) {}
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    std::vector<int32_t> axes_array;
    for (auto &use : op->getResult(0).getUses()) {
      if (auto tpuOp = llvm::dyn_cast<tpu::ReduceSumOp>(use.getOwner())) {
        nextOp = tpuOp.getOperation();
        if (tpuOp.axes().hasValue()) {
          arrayAttrToVector(tpuOp.axes().getValue(), axes_array);
        }
    ...
    LLVM_DEBUG(llvm::dbgs()
      << "ReshapeReduceSumPattern: " << getOpName(op)
      << ", layer ID " << getOpLayerId(op)
      << ", inputs " << inputShapes.size()
      << ", outputs " << outputShapes.size()
      << ", remove " << getOpName(op)
...
//   reshape: 1x1x32x32x32 -> 1x32x32x32
//   reduce_sum: 1x1x32x32x32 -> 1x32x32x32
struct TpuDReshapeReduceSumPattern : public RewritePattern {
  TpuDReshapeReduceSumPattern(MLIRContext *context)
      : RewritePattern(tpu::ReshapeOp::getOperationName(), 1, context) {}
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    LLVM_DEBUG(llvm::dbgs()
      << "DReshapeReduceSumPattern: " << getOpName(op)
      << ", layer ID " << getOpLayerId(op)
      << ", inputs " << inputShapes.size()
      << ", outputs " << outputShapes.size()
      << ", remove " << getOpName(op)
...
public:
  ...
  void runOnFunction() override {
    ...
    patterns.insert<TpuReshapeReduceSumPattern>(&getContext());
    patterns.insert<TpuDReshapeReduceSumPattern>(&getContext());
...
void tpu::ReshapeOp::getCanonicalizationPatterns(
  ...
  results.insert<
    ...
    TpuReshapeReduceSumPattern,
    TpuDReshapeReduceSumPattern,
