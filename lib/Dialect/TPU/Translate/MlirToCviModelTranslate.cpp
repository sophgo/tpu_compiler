  ...
  mainFunc_.walk([&](Operation *op) {
    if (op->getName().getDialect().str() != "tpu" || isa<tpu::NoneOp>(op) ||
      ...
      if (op->getAttr("preprocess")) {
        mlir::tpu::PreprocessParam preprocess = llvm::dyn_cast<tpu::InputOp>(op).preprocess();
        auto color = preprocess.color_order().getValue().str();
        auto raw_scale = preprocess.raw_scale().getValue().convertToFloat();;
        std::vector<float> mean;
        std::for_each(preprocess.mean().begin(), preprocess.mean().end(),
          [&](mlir::Attribute attr){ mean.push_back(attr.cast<FloatAttr>().getValue().convertToFloat()); });
        std::vector<float> std;
                std::for_each(preprocess.std().begin(), preprocess.std().end(),
          [&](mlir::Attribute attr){ std.push_back(attr.cast<FloatAttr>().getValue().convertToFloat()); });
        auto input_scale = preprocess.input_scale().getValue().convertToFloat();
