...
void MyMulOp::tiling(void *ctx, int64_t total) {
...
void MyMulOp::codeGenBf16(void *ctx,
                          std::vector<std::vector<int64_t>> &operand_shapes,
  ...
  for (auto &tile : tiles) {
    ...
    context->ops->tiu_mul(context, &p2);
...
RegisterCustomOp(MyMul, MyMulOp);
