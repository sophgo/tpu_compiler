...
parser = argparse.ArgumentParser(description="liveness caffe evaluation")
parser.add_argument('--model_def', type=str, default='',
                        help="Model definition file")
parser.add_argument('--pretrained_model', type=str, default='',
                        help='Load weights from previously saved parameters.')
parser.add_argument("--rgbir_val_list", type=str, help="The path of the rgbir file.",
                    default='')
parser.add_argument("--dataset", type=str, help="The root directory of the dataset.",
                    default="")
parser.add_argument("--is_caffe", type=bool, default=False)
...
labels = []
if args.is_caffe:
    module = caffe.Net(args.model_def, args.pretrained_model, caffe.TEST)
else:
    module = pymlir.module()
    module.load(args.model_def)
...
for i in tqdm(range(len(val_list))):
    ...
    for i in range(image.shape[0]):
        if args.is_caffe:
            module.blobs['data'].data[...] = np.expand_dims(image[i,:,:,:], 0)
            temp = module.forward()
            out[i] = copy.copy(temp['fc_blob2'])
        else:
