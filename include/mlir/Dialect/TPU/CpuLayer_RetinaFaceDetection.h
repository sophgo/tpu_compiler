#ifndef CPULAYER_RETINAFACE_DETECTION_H_
#define CPULAYER_RETINAFACE_DETECTION_H_


struct AnchorCfg {
public:
    AnchorCfg(int stride, std::vector<int> scales, int base_size, std::vector<float> ratios, int allowed_border)
        : stride(stride),
          scales(scales),
          base_size(base_size),
          ratios(ratios),
...
std::vector<AnchorBox> scale_enum(AnchorBox anchor, std::vector<int> &scales)
{
    ...
    for (size_t i = 0; i < scales.size(); ++i) {
        ...
        //std::cout << "x1 = " << scale_anchor.x1 << ",y1 = " << scale_anchor.y1
        //    << ",x2 = " << scale_anchor.x2 << ",y2 = " << scale_anchor.y2 << std::endl;
...
std::vector<std::vector<AnchorBox>> generate_anchors_fpn(bool dense, std::vector<AnchorCfg> &cfg)
{
...
std::vector<AnchorBox> anchors_plane(int height, int width, int stride, std::vector<AnchorBox> anchors_fpn)
{
    std::vector<AnchorBox> anchors;
    for(size_t k = 0; k < anchors_fpn.size(); ++k) {
        for(int ih = 0; ih < height; ++ih) {
            ...
            for(int iw = 0; iw < width; ++iw) {
                ...
                anchors.push_back(anchor);
                //std::cout << "x1 = " << anchor.x1 << ",y1 = " << anchor.y1
                //    << ",x2 = " << anchor.x2 << ",y2 = " << anchor.y2 << std::endl;
            }
        }
    }
...
std::vector<FaceInfo> nms(std::vector<FaceInfo> infos, float nms_threshold)
{
    ...
    std::sort(infos.begin(), infos.end(), [](FaceInfo &a, FaceInfo &b) { return a.score > b.score; });
